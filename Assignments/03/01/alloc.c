...
dopoolalloc(Pool *p, ulong asize, ulong pc)
{
	...
	t = p->root;
	q = nil; // Smallest but larger than the one we're searching.
	//printList(p);
	while(t) {
		if(t->size == size) {
			//t = t->fwd;
		...
		if(t->size < size) {
			t = t->nxt;
		} else { // size > t->size. Thats the largest.
			q = t;
			break;
		}
	}
