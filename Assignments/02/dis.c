...
Prog*
newprog(Prog *p, Modlink *m) // New user process
{
	Heap *h;
	Prog *n, **ph;
	Osenv *on, *op;
	...
	n = malloc(sizeof(Prog)+sizeof(Osenv)); // Allocate space for the new process
	if(n == 0){
		if(p == nil)
			panic("no memory");
		else
	...
	n->pid = ++pidnum;
	if(n->pid <= 0) // Overflow
		panic("Oh damn. What? It's been like a century! Did Trump destroy America?!?");
	n->group = nil;
	...
	ph = pidlook(n->pid);
	if(*ph != nil)
		panic("dup pid"); // This should not happen at all.
	n->pidlink = nil;
	*ph = n;
	...
	if(m->MP != H)
		Setmark(D2H(m->MP));
	addrun(n); // puts it on the run list

	if(p == nil){ // If there is no parent
		newgrp(n);
		print("No parent for this process\n");
		return n;
	}
...
int _ytab[2][12] = {{31,28,31,30,31,30,31,31,30,31,30,31},{31,29,31,30,31,30,31,31,30,31,30,31}};

#define	LEAPYEAR(year)	(!((year) % 4) && (((year) % 100) || !((year) % 400)))
#define	YEARSIZE(year)	(LEAPYEAR(year) ? 366 : 365)

char* toDateString(ulong time){
	ulong dayClock = time % (3600*24);
	ulong dayNo = time / (3600*24);

	int seconds = dayClock%60;
	int minutes = (dayClock%3600)/60;
	int hours = dayClock/3600;
	int weekDay = (dayNo+4)%7;
	int year = 1970;
	...
	asprintf(&date,"%d-%d-%d %d:%d:%d GMT",year,month,dayNo+1,hours,minutes,seconds);
	return date;
}
...
disinit(void *a)
{
	...
	print("User: %s\n", up->env->user);
	print("Kernel build epoch: %lud\n", kerndate);
	char* dateString = toDateString(kerndate);
	print("Kernel build string: %s\n", dateString);
	free(dateString);
